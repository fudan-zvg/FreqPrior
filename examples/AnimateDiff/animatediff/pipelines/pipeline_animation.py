...
from ..utils.util import save_videos_mp4
...
@dataclass
class AnimationFreqPriorPipelineOutput(BaseOutput):
    videos: Union[torch.Tensor, np.ndarray]
...
class AnimationFreqPriorPipeline(AnimationPipeline):
    _optional_components = []
    ...
    def __call__(
        ...
        **kwargs
        ...
        self.scheduler.set_timesteps(num_inference_steps, device=device)
        timesteps = self.scheduler.timesteps
        assert mid_step == 0 or mid_step in timesteps, (f"mid_step should be 0 or one of the timestep of schedulers!\n"
                                                        f"Scheduler timesteps: {timesteps.tolist()}")
        ...
        for iter in range(num_iters):
            ...
            if iter == 0:
                ...
                alpha_bar1 = self.scheduler.alphas_cumprod[current_diffuse_timestep]
                alpha_bar2 = 1 if mid_step == 0 else self.scheduler.alphas_cumprod[mid_step]
                s = alpha_bar1 / alpha_bar2
                z_T = s ** 0.5 * latents + (1 - s) ** 0.5 * initial_noise
                z_T = z_T.to(dtype=torch.float32)

                # 2. filtering
                eta1, eta2 = np.random.normal(size=z_T.shape), np.random.normal(size=z_T.shape)
                eta1 = torch.tensor(eta1, dtype=torch.float32, device=device)
                eta2 = torch.tensor(eta2, dtype=torch.float32, device=device)
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                ...
                for i, t in enumerate(current_timesteps):
                    if t == mid_step:
                        mid_latent = latents.detach().clone()
                        if (iter != num_iters - 1 and num_iters != 1) and not save_intermediate:
                            break
        ...
        return AnimationPipelineOutput(videos=videos)